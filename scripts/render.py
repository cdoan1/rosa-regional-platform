...
def render_shard_terraform(
    ...
    Creates:
    - deploy/<env>/<region_alias>/terraform/regional.yaml
    - deploy/<env>/<region_alias>/terraform/management/<cluster_id>.yaml
...
#
# This file is automatically generated by the render script.
# To make changes, edit argocd/config.yaml and run: scripts/render.py
#

"""

    # Generate regional.yaml
    regional_template = jinja_env.get_template('regional.yaml.j2')
    regional_file = terraform_dir / 'regional.yaml'
    with open(regional_file, 'w') as f:
        f.write(generated_header)
        f.write(regional_template.render(shard))

    print(f"  [OK] deploy/{environment}/{region_alias}/terraform/regional.yaml")
    ...
    if management_clusters:
        ...
        mc_template = jinja_env.get_template('management.yaml.j2')
        for mc in management_clusters:
            cluster_id = mc.get('cluster_id', '')

            mc_file = management_dir / f'{cluster_id}.yaml'
            with open(mc_file, 'w') as f:
                f.write(generated_header)
                f.write(mc_template.render(mc=mc, **shard))

            print(f"  [OK] deploy/{environment}/{region_alias}/terraform/management/{cluster_id}.yaml")
